...
new = '''        if (!chase && isDancerInvincible() && safeInt(run.invincibleRequestIndex) < run.invincibleObstaclePlanCount && run.elapsed >= spawn.nextInvincibleObstacleAt && !hasPendingInvincibleRequest()) {'''
count = text.count(old)
if count != 1:
    raise SystemExit(f'invincible request cap: expected 1, got {count}')
path.write_text(text.replace(old, new, 1))
print('8-second invincibility request count capped at the planned target')
